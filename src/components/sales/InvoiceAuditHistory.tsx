...
import {
  ...
  InvoiceAuditInfo,
} from '@/actions/sales/invoices/get-invoice-audit-history';
import {
  FileText,
  Calendar,
  Clock,
...
const AuditLogEntryRow = ({ entry }: { entry: InvoiceAuditLogEntry }) => {
  const displayUser = entry.user_name ?? 'Sistema';
  const createDeleteLabel = !entry.field_name && (entry.action_type === 'CREATE' || entry.action_type === 'DELETE') ? 'Acción' : null;
  return (
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-sm text-gray-700 mb-1">
          {entry.action_type === 'CREATE' && (
            <div>
              {createDeleteLabel && <span className="font-medium">{createDeleteLabel}: </span>}
...
export default function InvoiceAuditHistory({ invoiceId }: InvoiceAuditHistoryProps) {
  const [auditInfo, setAuditInfo] = useState<InvoiceAuditInfo | null>(null);
  ...
  const loadAuditHistory = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getInvoiceAuditHistory(invoiceId);
  ...
  if (!auditInfo) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-gray-500">No hay información de auditoría para esta factura.</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-green-50 p-4 rounded-lg">
            ...
            {auditInfo.seller_name && (
              <p className="text-xs text-green-600 mt-1">
                Vendedor: {auditInfo.seller_name}
              </p>
            )}
          </div>
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <p className="text-xs text-blue-600 mt-1">Factura {auditInfo.invoice_number}</p>
          </div>
        </div>
