...
async function corregirVentaREC000027() {
  console.log('🔧 Corrigiendo venta REC-000027...');
  
  try {
    // 1. Buscar la venta REC-000027
    const { data: venta, error: errorVenta } = await supabase
      .from('POSSale')
      .select('*')
      .eq('saleNumber', 'REC-000027')
      .single();
    
    if (errorVenta) {
      console.error('❌ Error buscando venta REC-000027:', errorVenta);
      return;
    }
    
    if (!venta) {
      console.error('❌ Venta REC-000027 no encontrada');
      return;
    }
    ...
    if (venta.clientId) {
      ...
      if (cliente) {
        ...
        console.log('✅ Venta REC-000027 corregida exitosamente');
