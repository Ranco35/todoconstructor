-- Migración oficial para agregar tablas de clientes
-- Esta migración se puede aplicar desde la interfaz de Supabase

-- Crear tablas y relaciones para la interfaz de clientes
-- Usar IF NOT EXISTS y comprobaciones para evitar errores si ya existen
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Enable all for service role' AND tablename = 'Country'
  ) THEN
    EXECUTE 'CREATE POLICY "Enable all for service role" ON "Country" FOR ALL USING (true)';
  END IF;
END $$;

-- Repite la lógica para las demás políticas y tablas... 