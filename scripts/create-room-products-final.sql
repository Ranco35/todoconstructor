...
-- CORREGIDO: Usa la estructura real de ambas tablas
...
SELECT 
  ...
  COALESCE(r.amenities, 'Habitación ' || r.number || ' de tipo ' || r.type) as description,
...
SET 
  ...
  description = COALESCE(r.amenities, 'Habitación ' || r.number || ' de tipo ' || r.type),
  "updatedAt" = NOW()
FROM rooms r
